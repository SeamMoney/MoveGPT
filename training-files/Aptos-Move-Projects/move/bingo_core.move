module overmind::bingo_core {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (resource_account, signCap) = account::create_resource_account(admin, BINGO_SEED);
        ...
        move_to(admin, State { bingo: signer::address_of(&resource_account) });
        // TODO: Move Bingo resource to the resource account's address
        move_to(&resource_account, Bingo {
            games: simple_map::create<String, Game>(),
            cap: signCap,
            create_game_events: account::new_event_handle<CreateGameEvent>(&resource_account),
            cancel_game_events: account::new_event_handle<CancelGameEvent>(&resource_account)
        })
    }
    ...
    */
    public entry fun create_game(
        ...
        let game = Game {
            ...
            is_finished: false,
            insert_number_events: account::new_event_handle<InsertNumberEvent>(&account::create_signer_with_capability(&bingo.cap)),
            join_game_events: account::new_event_handle<JoinGameEvent>(&account::create_signer_with_capability(&bingo.cap)),
            bingo_events: account::new_event_handle<BingoEvent>(&account::create_signer_with_capability(&bingo.cap))
        };
        // TODO: Add the game to the bingo's game list
        simple_map::add(&mut bingo.games,game_name, game);
        // TODO: Emit CreateGameEvent event
        event::emit_event(&mut bingo.create_game_events, bingo_events::new_create_game_event(game_name, entry_fee, start_timestamp,timestamp::now_seconds()));
    }
    ...
    */
    public entry fun insert_number(admin: &signer, game_name: String, number: u8) acquires State, Bingo {
        ...
        event::emit_event(&mut game.insert_number_events, bingo_events::new_inser_number_event(game_name, number, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun join_game(player: &signer, game_name: String, numbers: vector<vector<u8>>) acquires State, Bingo {
        ...
        assert_suffiecient_funds_to_join(addr, game.entry_fee);

        // TODO: Assert that the player has not joined the game yet
        assert_player_not_joined_yet(&game.players, &addr);

        // TODO: Add the player to the game's list of players
        simple_map::add(&mut game.players, addr, numbers);
        ...
        event::emit_event(&mut game.join_game_events, bingo_events::new_join_game_event(game_name, addr,numbers, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun bingo(player: &signer, game_name: String) acquires State, Bingo {
        ...
        assert_player_joined(&game.players, &addr);
        // TODO: Assert that the player has bingo
        assert_player_has_bingo(&game.drawn_numbers, *simple_map::borrow(&game.players, &addr));

        // TODO: Change the game's is_finished field's value to true
        game.is_finished = true;

        // TODO: Transfer all players' entry fees to the winner
        let n = simple_map::length(&game.players);
        coin::transfer<AptosCoin>(&account::create_signer_with_capability(&bingo.cap), addr, game.entry_fee*n);

        // TODO: Emit BingoEvent event
        event::emit_event(&mut game.bingo_events, bingo_events::new_bingo_event(game_name, addr, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun cancel_game(admin: &signer, game_name: String) acquires State, Bingo {
        ...
        let n = simple_map::length(&game.players);
        let i = 0;
        let (addr, _) = simple_map::to_vec_pair(game.players);
        while (i < n){
            let player = vector::borrow(&addr, i);
            coin::transfer<AptosCoin>(&account::create_signer_with_capability(&bingo.cap), *player, game.entry_fee);
            i = i + 1;
        };

        // TODO: Emit CancelGameEvent event
        event::emit_event(&mut bingo.cancel_game_events, bingo_events::new_cance_game_event(game_name, timestamp::now_seconds()));
    }
    ...
    */
    fun check_player_numbers(drawn_numbers: &vector<u8>, player_numbers: vector<vector<u8>>): bool {
        ...
        let player_numbers_new = vector::empty<vector<Option<u8>>>();
        let i = 0;
        while (i < 5){
            let j = 0;
            let row = vector::empty<Option<u8>>();
            while (j < 5){
                let number = *vector::borrow(vector::borrow(&player_numbers,i),j);
                if (number == 0){
                    vector::push_back(&mut row, option::none<u8>());
                }
                else if (vector::contains(drawn_numbers,&number)){
                    vector::push_back(&mut row, option::none<u8>());
                }
                else{
                    vector::push_back(&mut row, option::some<u8>(number));
                };
                j = j + 1;
            };
            vector::push_back(&mut player_numbers_new, row);
            i = i + 1;
        };

        // TODO: Call check_columns, check_diagonals and check_rows and return true if any of those returns true
        return check_columns(&player_numbers_new) || check_diagonals(&player_numbers_new) || check_rows(&player_numbers_new)
    }
    ...
    */
    inline fun check_columns(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        let ans = vector [true, true, true, true, true];
        let i = 0;
        let a = false;
        while (i < 5){
            let j = 0;
            while (j < 5){
                ...
                if (option::is_some(&number)){
                    let elem = vector::borrow_mut(&mut ans, i);
                    *elem = false;
                };
                j = j + 1;
            };
    ...
    inline fun check_rows(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        while (i < 5){
            ...
            while (j < 5){
                let number = *vector::borrow(vector::borrow(player_numbers,i),j);
                if (option::is_some(&number)){
                    let elem = vector::borrow_mut(&mut ans, j);
                    *elem = false;
                };
                j = j + 1;
            };
    ...
    */
    inline fun check_diagonals(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        while (i < 5){
            ...
            while (j < 5){
                if (i == j){
                    if (option::is_some(&*vector::borrow(vector::borrow(player_numbers,i),j))){
                        first = false;
                    };
                };
                if (i + j == 4){
    ...
    inline fun assert_numbers_are_picked_correctly(picked_numbers: &vector<vector<u8>>) {
        ...
        while (i < 5){
            let j = 0;
            while (j < 5){
                let number = *vector::borrow(vector::borrow(picked_numbers,i),j);
                if (i == 0) {
                    assert!(number >= 1 && number <= 15, COLUMN_HAS_INVALID_NUMBER);
                } else if (i == 1) {
                    assert!(number >= 16 && number <= 30, COLUMN_HAS_INVALID_NUMBER);
                } else if (i == 2) {
                    if (j == 2) {
                        assert!(number == 0, COLUMN_HAS_INVALID_NUMBER);
                    } else {
                        assert!(number >= 31 && number <= 45, COLUMN_HAS_INVALID_NUMBER);
                    }
                } else if (i == 3) {
                    assert!(number >= 46 && number <= 60, COLUMN_HAS_INVALID_NUMBER);
                } else if (i == 4) {
                    assert!(number >= 61 && number <= 75, COLUMN_HAS_INVALID_NUMBER);
                };
                j = j + 1;
            };
            i = i + 1;
