module collectibleswap::iterable_table {
    ...
    public fun get_keys<K: copy + store + drop, V: store>(table: &IterableTable<K, V>): vector<K> {
        let key = head_key(table);
        let keys = vector::empty<K>();
        while (option::is_some(&key)) {
            let (_, _, next) = borrow_iter(table, *option::borrow(&key));
            vector::push_back(&mut keys, *option::borrow(&key));
            key = next;
        };
        keys
    }
