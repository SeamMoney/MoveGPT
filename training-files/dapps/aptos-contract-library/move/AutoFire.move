module MasterChefDepolyer::AutoFire {
    use std::signer;
    use aptos_std::event;
    use aptos_framework::coin::{Self};
    use aptos_framework::timestamp;
    use aptos_framework::account::{Self, SignerCapability};
    use MasterChefDepolyer::FireMasterChefV1::{Self, FIRE};
    ...
    const FIRE_PRECISION: u128 = 1000000000000;  // 1e12
    const DEPLOYER: address = @Depolyer;
    ...
    struct UserInfo has key, store {
        ...
        last_user_action_FIRE: u64,   // keeps track of FIRE deposited at the last user action
        last_user_action_time: u64,  // keeps track of the last user action time
    }

    struct AutoFIREData has drop, key {
    ...
    fun get_resource_account(): signer acquires AutoFIREData {
        let signer_cap = &borrow_global<AutoFIREData>(DEPLOYER).signer_cap;
        account::create_signer_with_capability(signer_cap)
    }

    // return pair admin account address
    fun get_resource_account_address(): address acquires AutoFIREData {
    ...
    fun init_module(admin: &signer) {
        ...
        // register
        FireMasterChefV1::register_FIRE(admin);
    ...
    fun when_paused() acquires AutoFIREData {
        assert!(borrow_global<AutoFIREData>(DEPLOYER).is_pause == true, PAUSABLE_ERROR);
    }

    fun when_not_paused() acquires AutoFIREData {
        assert!(borrow_global<AutoFIREData>(DEPLOYER).is_pause == false, PAUSABLE_ERROR);
    }

    // Deposits funds into the FIRE Vault
    public entry fun deposit(
        account: &signer,
        amount: u64
    ) acquires AutoFIREData, UserInfo, Events {
        when_not_paused();
        assert!(amount > 0, AMOUNT_ERROR);
        let auto_data = borrow_global_mut<AutoFIREData>(DEPLOYER);
        ...
        let pool_balance = balance_of(resource_account_address);
        coin::transfer<FIRE>(account, resource_account_address, amount);
        let current_shares;
        if (auto_data.total_shares != 0) {
            current_shares = (amount as u128) * auto_data.total_shares / (pool_balance as u128);
        } else {
        ...
        if (exists<UserInfo>(acc_addr)) {
            ...
            user_info.last_user_action_FIRE = (user_info.shares * (balance_of(resource_account_address) as u128) / auto_data.total_shares as u64);
            user_info.last_user_action_time = timestamp::now_seconds();
        } else {
            let user_info = UserInfo {
                shares: current_shares,
                last_deposited_time: timestamp::now_seconds(),
                last_user_action_FIRE: (current_shares * (balance_of(resource_account_address) as u128) / auto_data.total_shares as u64),
    ...
    public entry fun withdraw(
        ...
        if (bal < current_amount) {
            ...
            FireMasterChefV1::leave_staking(&resource_account_signer, (bal_withdraw as u64));
        ...
        if (timestamp::now_seconds() < user_info.last_deposited_time + auto_data.withdraw_fee_period) {
            ...
            coin::transfer<FIRE>(&resource_account_signer, auto_data.treasury, (current_withdraw_fee as u64));
        ...
        if (user_info.shares > 0) {
            ...
            user_info.last_user_action_FIRE = 0;
        };

        user_info.last_user_action_time = timestamp::now_seconds();
        coin::transfer<FIRE>(&resource_account_signer, acc_addr, (current_amount as u64));
        // emit withdraw
        let events = borrow_global_mut<Events>(resource_account_address);
    ...
    // Reinvests tokens into MasterChef
    public entry fun harvest(
        ...
        FireMasterChefV1::leave_staking(&resource_account_signer, 0);
        let bal = (available(resource_account_address) as u128);
        let current_performance_fee = bal * (auto_data.performance_fee as u128) / 10000;
        coin::transfer<FIRE>(&resource_account_signer, auto_data.treasury, (current_performance_fee as u64));

        let current_call_fee = bal * (auto_data.call_fee as u128) / 10000;
        coin::transfer<FIRE>(&resource_account_signer, acc_addr, (current_call_fee as u64));
    ...
    fun earn_internal(
        ...
        if (bal > 0) {
            FireMasterChefV1::enter_staking(resource_account_signer, bal);
        }
    }
    ...
    fun balance_of(
        ...
        let amount = FireMasterChefV1::get_user_info_amount<FIRE>(addr);
        coin::balance<FIRE>(addr) + amount
    }

    public entry fun set_admin_address(
        admin: &signer,
        new_admin_address: address
    ) acquires AutoFIREData {
        let aa_data = borrow_global_mut<AutoFIREData>(DEPLOYER);
    ...
    public entry fun emergency_withdraw(
        ...
        let aa_data = borrow_global_mut<AutoFIREData>(DEPLOYER);
        assert!(signer::address_of(admin) == aa_data.admin_address, FORBIDDEN);
        FireMasterChefV1::emergency_withdraw<FIRE>(&get_resource_account());
    }
    ...
    public fun calculate_total_pending_FIRE_rewards(): u64 acquires AutoFIREData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);

        let amount = FireMasterChefV1::pending_FIRE<FIRE>(resource_account_address);
        amount = amount + available(resource_account_address);
        amount
    }

    // Calculates the expected harvest reward from third party
    public fun calculate_harvest_FIRE_rewards(): u64 acquires AutoFIREData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);

        let amount = FireMasterChefV1::pending_FIRE<FIRE>(resource_account_address);
        let auto_data = borrow_global<AutoFIREData>(DEPLOYER);
        let current_call_fee = (amount as u128) * (auto_data.call_fee as u128) / 10000;
        (current_call_fee as u64)
    }

    public fun get_price_per_full_share(): u64 acquires AutoFIREData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);
        let auto_data = borrow_global<AutoFIREData>(DEPLOYER);
        
        if (auto_data.total_shares == 0) {
            return (FIRE_PRECISION as u64)
        } else {
            let amount = (balance_of(resource_account_address) as u128) * FIRE_PRECISION / auto_data.total_shares;
            return (amount as u64)
        }
    }

    public fun get_auto_FIRE_data(): (u128, u64, u64, u64, u64, u64) acquires AutoFIREData {
        let auto_data = borrow_global<AutoFIREData>(DEPLOYER);
    ...
    public fun test_init(creator: &signer, someone_else: &signer) {
        FireMasterChefV1::test_init(creator, someone_else);
        init_module(creator);
    }

    #[test_only]
    public fun test_init_another(creator: &signer, someone_else: &signer, another_one: &signer) {
        FireMasterChefV1::test_init_another(creator, someone_else, another_one);
        init_module(creator);
    }

    #[test(creator = @Depolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_1(creator: &signer, someone_else: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;
        let (_total_shares, _performance_fee, _call_fee, withdraw_fee, _withdraw_fee_period, _last_harvested_time) = get_auto_FIRE_data();

        deposit(someone_else, amount);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        withdraw_all(someone_else);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN - amount * withdraw_fee / 10000, TEST_ERROR);
    }

    #[test(creator = @Depolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_2(creator: &signer, someone_else: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;
        let (_total_shares, _performance_fee, _call_fee, _withdraw_fee, _withdraw_fee_period, _last_harvested_time) = get_auto_FIRE_data();

        deposit(someone_else, amount);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        timestamp::fast_forward_seconds(_withdraw_fee_period + 1);
        withdraw_all(someone_else);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN, TEST_ERROR);
    }

    #[test(creator = @Depolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_3(creator: &signer, someone_else: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;

        deposit(someone_else, amount * 2);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN - 2 * amount, TEST_ERROR);
        timestamp::fast_forward_seconds(86400 * 10);
        withdraw(someone_else, (amount as u128));
        withdraw_all(someone_else);
        let balance = coin::balance<FIRE>(signer::address_of(someone_else));
        debug::print(&balance);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN + 86400 * 10 * 97 / 100 * 10 * 6000000, TEST_ERROR);
    }

    #[test(creator = @Depolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_1(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;

        deposit(someone_else, amount);
        FireMasterChefV1::enter_staking(another_one, amount);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        assert!(coin::balance<FIRE>(signer::address_of(another_one)) == INIT_COIN - amount, TEST_ERROR);
        ...
        timestamp::fast_forward_seconds(86400 * 5);
        withdraw_all(someone_else);
        FireMasterChefV1::leave_staking(another_one, amount);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) > coin::balance<FIRE>(signer::address_of(another_one)), TEST_ERROR);
    }

    #[test(creator = @Depolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_2(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;
    ...
    #[test(creator = @Depolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_3(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;
        ...
        deposit(someone_else, amount);
        FireMasterChefV1::enter_staking(another_one, amount);
        assert!(FireMasterChefV1::pending_FIRE<FIRE>(signer::address_of(another_one)) == 0, TEST_ERROR);

        timestamp::fast_forward_seconds(86400 * 5);
        FireMasterChefV1::leave_staking(another_one, amount * 2);
        withdraw_all(someone_else);
        assert!(coin::balance<FIRE>(signer::address_of(someone_else)) < coin::balance<FIRE>(signer::address_of(another_one)), TEST_ERROR);
    }
    ...
    public entry fun test_pause_1(creator: &signer, someone_else: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init(creator, someone_else);
        pause(creator);
    ...
    #[test(creator = @Depolyer, someone_else = @0x11)]
    public entry fun test_pause_2(creator: &signer, someone_else: &signer) acquires AutoFIREData, UserInfo, Events {
        test_init(creator, someone_else);
        pause(creator);
        unpause(creator);
        deposit(someone_else, 10000);
