module SwapDeployer::AtodexSwapPoolV1 {
    use ResourceAccountDeployer::LPCoinV1::LPCoin;
    use SwapDeployer::AtodexSwapPoolV1Library;
    ...
    public fun get_reserves_size<X, Y>(): (u64, u64) acquires LiquidityPool {
        if (AtodexSwapPoolV1Library::compare<X, Y>()) {
    ...
    public fun get_amounts_out_1_pair<X, Y>(
        ...
        let amount_out = AtodexSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        amount_out
    }
    ...
    public fun get_amounts_out_2_pair<X, Y, Z>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_mid = AtodexSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_out = AtodexSwapPoolV1Library::get_amount_out(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_out
    }
    ...
    public fun get_amounts_out_3_pair<X, Y, Z, W>(
        ...
        let amount_mid = AtodexSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_mid = AtodexSwapPoolV1Library::get_amount_out(amount_mid, reserve_in, reserve_out, swap_fee);
    ...
    public fun get_amounts_in_1_pair<X, Y>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_in = AtodexSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun get_amounts_in_2_pair<X, Y, Z>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_mid = AtodexSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_in = AtodexSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun get_amounts_in_3_pair<X, Y, Z, W>(
        ...
        let amount_mid = AtodexSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_mid = AtodexSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Z, W>();
        let amount_in = AtodexSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun calc_optimal_coin_values<X, Y>(
        ...
        if (reserve_x == 0 && reserve_y == 0) {
            ...
            let amount_y_optimal = AtodexSwapPoolV1Library::quote(amount_x_desired, reserve_x, reserve_y);
            if (amount_y_optimal <= amount_y_desired) {
                ...
                let amount_x_optimal = AtodexSwapPoolV1Library::quote(amount_y_desired, reserve_y, reserve_x);
    ...
    fun assert_k_increase(
        ...
        // gas saving
        if (
            AtodexSwapPoolV1Library::is_overflow_mul(balance_x_adjusted, balance_y_adjusted)
            || AtodexSwapPoolV1Library::is_overflow_mul(balance_xy_old_not_scaled, scale)
        ) {
    ...
    fun update_internal<X, Y>(
        ...
        if (time_elapsed > 0 && reserve_x != 0 && reserve_y != 0) {
            ...
            lp.last_price_x_cumulative = AtodexSwapPoolV1Library::overflow_add(lp.last_price_x_cumulative, last_price_x_cumulative_delta);

            let last_price_y_cumulative_delta = uq64x64::to_u128(uq64x64::fraction(reserve_x, reserve_y)) * time_elapsed;
            lp.last_price_y_cumulative = AtodexSwapPoolV1Library::overflow_add(lp.last_price_y_cumulative, last_price_y_cumulative_delta);
        };
    ...
    fun mint_fee_interval<X, Y>(
        ...
        if (fee_on) {
            if (k_last != 0) {
                ...
                let root_k = AtodexSwapPoolV1Library::sqrt(reserve_x, reserve_y);
                let root_k_last = AtodexSwapPoolV1Library::sqrt_128(k_last);
                let total_supply = AtodexSwapPoolV1Library::get_lpcoin_total_supply<LPCoin<X, Y>>();
                if (root_k > root_k_last) {
                    let delta_k = ((root_k - root_k_last) as u128);
                    // gas saving
                    if (AtodexSwapPoolV1Library::is_overflow_mul(total_supply, delta_k)) {
    ...
    public entry fun create_pair_entry<X, Y>(
        account: &signer
    ) acquires AdminData, PairInfo {
        if (AtodexSwapPoolV1Library::compare<X, Y>()) {
            create_pair<X, Y>(account);
        } else {
            create_pair<Y, X>(account);
        }
    }
    ...
    public entry fun remove_liquidity_entry<X, Y>(
        ...
        if (AtodexSwapPoolV1Library::compare<X, Y>()) {
            ...
            (x_out, y_out) = remove_liquidity<X, Y>(account, coins, amount_x_min, amount_y_min);
        } else {
            let coins = coin::withdraw<LPCoin<Y, X>>(account, liquidity);
            (y_out, x_out) = remove_liquidity<Y, X>(account, coins, amount_y_min, amount_x_min);
        };
        // transfer
    ...
    public entry fun swap_exact_coins_for_coins_entry<X, Y>(
        ...
        let coins_out;
        coins_out = swap_coins_for_coins<X, Y>(account, coins_in);
        assert!(coin::value(&coins_out) >= amount_out_min, ERR_INSUFFICIENT_OUTPUT_AMOUNT);
        AtodexSwapPoolV1Library::register_coin<Y>(account);
    ...
    public entry fun swap_exact_coins_for_coins_2_pair_entry<X, Y, Z>(
        ...
        let coins_out;
        let coins_mid;
        coins_mid = swap_coins_for_coins<X, Y>(account, coins_in);
        coins_out = swap_coins_for_coins<Y, Z>(account, coins_mid);
        assert!(coin::value(&coins_out) >= amount_out_min, ERR_INSUFFICIENT_OUTPUT_AMOUNT);
        AtodexSwapPoolV1Library::register_coin<Z>(account);
    ...
    public entry fun swap_exact_coins_for_coins_3_pair_entry<X, Y, Z, W>(
        ...
        let coins_mid_2;
        coins_mid = swap_coins_for_coins<X, Y>(account, coins_in);
        coins_mid_2 = swap_coins_for_coins<Y, Z>(account, coins_mid);
        coins_out = swap_coins_for_coins<Z, W>(account, coins_mid_2);
        assert!(coin::value(&coins_out) >= amount_out_min, ERR_INSUFFICIENT_OUTPUT_AMOUNT);
        AtodexSwapPoolV1Library::register_coin<W>(account);
    ...
    /// require X < Y
    public fun create_pair<X, Y>(account: &signer) acquires AdminData, PairInfo {
        assert!(AtodexSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
        ...
        // create lp coin
        let (lp_b, lp_f, lp_m) = coin::initialize<LPCoin<X, Y>>(&resource_account_signer, utf8(b"AtodexLPCoin"), utf8(b"ADXLP"), 8, true);
        // register coin
        AtodexSwapPoolV1Library::register_coin<LPCoin<X, Y>>(&resource_account_signer);
    ...
    public fun add_liquidity<X, Y>(
        ...
        let lp_coins = mint<X, Y>(account, coin_x, coin_y);
    ...
    public fun remove_liquidity<X, Y>(
        ...
        assert!(AtodexSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
        let (x_out, y_out) = burn<X, Y>(account, coins);
    ...
    public fun swap_coins_for_coins<X, Y>(
        ...
        let (zero, coins_out);
        if (AtodexSwapPoolV1Library::compare<X, Y>()) {
            (zero, coins_out) = swap<X, Y>(account, coins_in, 0, coin::zero(), amount_out);
        } else {
            (coins_out, zero) = swap<Y, X>(account, coin::zero(), amount_out, coins_in, 0);
        };
    ...
    public fun mint<X, Y>(
        ...
        let liquidity;
        if (total_supply == 0) {
            liquidity = AtodexSwapPoolV1Library::sqrt(amount_x, amount_y) - MINIMUM_LIQUIDITY;
            ...
            liquidity = AtodexSwapPoolV1Library::min(amount_1, amount_2);
        };
    ...
    fun init_module_test(resource_account: &signer) acquires AdminData, PairInfo {
        ...
        create_pair<std::aptos_coin::AptosCoin, BTC>(resource_account);
        create_pair<std::aptos_coin::AptosCoin, USDT>(resource_account);
    }
    ...
    public entry fun test_add_remove_liquidity_basic_1_1(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 10000/10000 coin and gives 9000 LPCoin (AtodexSwapPoolV1Library::sqrt(10000*10000)-1000)
    ...
    public entry fun test_add_remove_liquidity_basic_1_100(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 1000/100000 coin and gives 9000 LPCoin (AtodexSwapPoolV1Library::sqrt(1000*100000)-1000)
    ...
    public entry fun test_add_remove_liquidity_basic_1_2(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 1000/2000 coin and gives 414 LPCoin (AtodexSwapPoolV1Library::sqrt(1000*2000)-1000)
    ...
    public entry fun test_flash_swap_2(creator: &signer, resource_account: &signer, someone_else: &signer, another_one : &signer)
        ...
        pay_flash_swap<BTC, USDT>(another_one, repay_coin_x, repay_coin_y, flash_swap);
        {
            let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
    ...
    public entry fun test_flash_swap_3(creator: &signer, resource_account: &signer, someone_else: &signer, another_one : &signer)
        ...
        coin::deposit<USDT>(signer::address_of(another_one), coin_out_2);
        let repay_coin_x = coin::withdraw<BTC>(another_one, 1004);
        pay_flash_swap<BTC, USDT>(another_one, repay_coin_x, coin::zero<USDT>(), flash_swap);
        {
            let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
            assert!(coin::value(&lp.coin_x_reserve) == 10004, CONTRACTOR_BALANCE_ERROR);
            assert!(coin::value(&lp.coin_y_reserve) == 10000, CONTRACTOR_BALANCE_ERROR);
