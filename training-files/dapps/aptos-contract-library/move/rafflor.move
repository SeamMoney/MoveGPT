...
module aptos_monkeys_rafflor::rafflor {
    use std::error;
    use std::signer;
    use std::vector;
    use std::timestamp;
    ...
    const ETICKET_MAX_PER_WALLET_REACHED: u64 = 13;
    const EARGS_ERROR: u64 = 14;
    const EONLY_RAFFLOR_CAN_CANCEL: u64 = 15;
    const ETOO_LATE_TO_BUY_TICKETS: u64 = 16;
    const ETICKET_COUNT_TOO_LOW: u64 = 17;
    ...
    public entry fun create_forum<CoinType>(sender: &signer, fee_percentage: u64) {
        ...
        let (_, signer_cap) = account::create_resource_account(sender, x"01f7a8");
    ...
    public entry fun create_raffle<CoinType>(sender: &signer, forum_address: address, creator: address, collection: String, name: String, property_version: u64, ticket_price: u64, ticket_max: u64, ticket_max_per_wallet: u64, draw_time: u64) acquires Forum, ForumEvents, RaffleStore {
        let forum_id = ForumId { forum_address };
        ...
        assert!(ticket_price > 0, EARGS_ERROR);
        assert!(ticket_max > 0, EARGS_ERROR);
        assert!(ticket_max_per_wallet > 0, EARGS_ERROR);
        assert!(ticket_max_per_wallet <= ticket_max, EARGS_ERROR);
    ...
    public entry fun buy_tickets<CoinType>(sender: &signer, forum_address: address, raffle_id: u64, ticket_count: u64) acquires Forum, ForumEvents, RaffleStore, TicketsEnvelope {
        let forum_id = ForumId { forum_address };
        let forum = borrow_global<Forum>(forum_address);
        let forum_events = borrow_global_mut<ForumEvents>(forum_address);
        let raffle_store = borrow_global_mut<RaffleStore<CoinType>>(forum_address);
        ...
        assert!((timestamp::now_seconds() * 1000) < raffle.draw_time, ETOO_LATE_TO_BUY_TICKETS);
        ...
        let wallet_tickets = &mut tickets_envelope.tickets;
        let wallet_ticket_count = 0;
        let i = 0;

        while (i < vector::length(wallet_tickets)) {
            let ticket = vector::borrow(wallet_tickets, i);
            if (ticket.raffle_id == raffle_id) {
                wallet_ticket_count = wallet_ticket_count + 1;
            };
            i = i + 1;
        };

        assert!(wallet_ticket_count + ticket_count <= raffle.ticket_max_per_wallet, ETICKET_MAX_PER_WALLET_REACHED);

        let tickets_price = raffle.ticket_price * ticket_count;
        let fee = tickets_price * forum.fee_percentage / 100;

        coin::transfer<CoinType>(sender, @aptos_monkeys_rafflor, fee);
        coin::transfer<CoinType>(sender, raffle.rafflor, tickets_price - fee);
        ...
        while (j <= ticket_count) {
            let ticket_id = lastTicketId + j;

            vector::push_back(wallet_tickets, Ticket {
                raffle_id,
                ticket_id,
                forum_id,
            });
    ...
    public entry fun draw_raffle<CoinType>(sender: &signer, forum_address: address, raffle_id: u64) acquires Forum, ForumEvents, RaffleStore {
        ...
        if(raffle.ticket_sold == raffle.ticket_max) {
            raffle.draw_time = timestamp::now_seconds() * 1000;
        };

        assert!(raffle.drawn == false, ERAFFLE_ALREADY_DRAWN);
        assert!((timestamp::now_seconds() * 1000) >= raffle.draw_time, EDRAW_TIME_NOT_REACHED);

        if(raffle.ticket_sold == 0) {
            if(raffle.rafflor == sender_addr) {
                raffle.drawn = true;
        ...
        assert!(raffle.ticket_sold > 1, ETICKET_COUNT_TOO_LOW);
