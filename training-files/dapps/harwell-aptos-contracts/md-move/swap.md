...
module hwswap::swap {
    ...
    use hwswap::swap_utils;
    use hwswap::u256;

    friend hwswap::router;

    const ZERO_ACCOUNT: address = @zero;
    const DEFAULT_ADMIN: address = @hwswap_admin;
    const RESOURCE_ACCOUNT: address = @hwswap;
    ...
    */
    ...
    public entry fun emergency_withdraw<CoinType>(sender: &signer,to: address, amount: u64) acquires SwapInfo{
        assert!(signer::address_of(sender)==DEFAULT_ADMIN,ERROR_NOT_ADMIN);
    ...
    public(friend) fun create_pair<X, Y>(
        ...
        let lp_name: string::String = string::utf8(b"HW-");
        ...
        move_to<TokenPairReserve<X, Y>>( &resource_signer, TokenPairReserve {
            reserve_x: 0,
            reserve_y: 0,
            block_timestamp_last: 0
        });

        move_to<TokenPairMetadata<X, Y>>( &resource_signer, TokenPairMetadata {
            creator: sender_addr,
        ...
        move_to<PairEventHolder<X, Y>>( &resource_signer, PairEventHolder {
        ...
        event::emit_event<PairCreatedEvent>( &mut swap_info.pair_created, PairCreatedEvent {
            user: sender_addr,
            token_x,
            token_y
        });
    ...
    fun add_liquidity_direct<X, Y>(
        ...
        let (a_x,a_y) = swap_utils::get_mint_x_y(reserve_x,reserve_y,amount_x,amount_y);
    ...
    fun mint<X, Y>(): (coin::Coin<LPToken<X, Y>>, u64) acquires TokenPairReserve, TokenPairMetadata {
        ...
        let liquidity = swap_utils::get_liquidity(
    ...
    fun burn<X, Y>(lp_tokens: coin::Coin<LPToken<X, Y>>): (coin::Coin<X>, coin::Coin<Y>, u64) acquires TokenPairMetadata, TokenPairReserve {
        ...
        //Need to add fee amount which have not been mint.
        let (amount_x,amount_y) = swap_utils::get_burn_x_y(balance_x,balance_y,total_lp_supply<X,Y>(),liquidity);
    ...
    fun mint_fee<X, Y>(reserve_x: u64, reserve_y: u64, metadata: &mut TokenPairMetadata<X, Y>): u64 {
        let fee = swap_utils::get_fee(reserve_x,reserve_y,metadata.k_last,total_lp_supply<X,Y>());
        if (fee > 0) {
