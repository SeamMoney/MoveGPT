/// Optimistic Voting Module based-on aptos_framework::voting
/// Change the logic of get_proposal_state to following condition
/// Only failed when yes_votes + no_votes >= threshold and no_votes >= yes_votes
/// Otherwise is successed 
module injoy_labs::op_voting {
    ...
    public fun get_proposal_state<ProposalType: store>(
        ...
        if (is_voting_closed<ProposalType>(voting_forum_address, proposal_id)) {
            ...
            // Optimisitc Logic modified by InJoy Labs
            if (no_votes >= yes_votes && yes_votes + no_votes >= proposal.min_vote_threshold)
            {
