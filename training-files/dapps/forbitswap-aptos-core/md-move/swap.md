```rust
module SwapDeployer::ForbitSwapV1 {
    ...
    public entry fun update_module(admin: &signer) {
        // init admin data
        let signer_cap = LPResourceAccount::retrieve_signer_cap(admin);
        let resource_account = &account::create_signer_with_capability(&signer_cap);
        move_to(resource_account, AdminData {
    ...
    public fun get_amounts_out_1_pair<X, Y>(
        ...
        let amount_out = ForbitSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        amount_out
    }
    ...
    public fun get_amounts_out_2_pair<X, Y, Z>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_mid = ForbitSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_out = ForbitSwapPoolV1Library::get_amount_out(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_out
    }
    ...
    public fun get_amounts_out_3_pair<X, Y, Z, W>(
        ...
        let amount_mid = ForbitSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_mid = ForbitSwapPoolV1Library::get_amount_out(amount_mid, reserve_in, reserve_out, swap_fee);
    ...
    public fun get_amounts_in_1_pair<X, Y>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_in = ForbitSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun get_amounts_in_2_pair<X, Y, Z>(
        ...
        let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
        let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
        let amount_mid = ForbitSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_in = ForbitSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun get_amounts_in_3_pair<X, Y, Z, W>(
        ...
        let amount_mid = ForbitSwapPoolV1Library::get_amount_in(amount_out, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Y, Z>();
        let amount_mid = ForbitSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        (reserve_in, reserve_out) = get_reserves_size<Z, W>();
        let amount_in = ForbitSwapPoolV1Library::get_amount_in(amount_mid, reserve_in, reserve_out, swap_fee);
        amount_in
    }
    ...
    public fun calc_optimal_coin_values<X, Y>(
        ...
        if (reserve_x == 0 && reserve_y == 0) {
            ...
            let amount_y_optimal = ForbitSwapPoolV1Library::quote(amount_x_desired, reserve_x, reserve_y);
            if (amount_y_optimal <= amount_y_desired) {
                ...
                let amount_x_optimal = ForbitSwapPoolV1Library::quote(amount_y_desired, reserve_y, reserve_x);
    ...
    fun assert_k_increase(
        ...
        // gas saving
        if (
            ForbitSwapPoolV1Library::is_overflow_mul(balance_x_adjusted, balance_y_adjusted)
            || ForbitSwapPoolV1Library::is_overflow_mul(balance_xy_old_not_scaled, scale)
        ) {
    ...
    fun update_internal<X, Y>(
        ...
        if (time_elapsed > 0 && reserve_x != 0 && reserve_y != 0) {
            ...
            lp.last_price_x_cumulative = ForbitSwapPoolV1Library::overflow_add(lp.last_price_x_cumulative, last_price_x_cumulative_delta);

            let last_price_y_cumulative_delta = uq64x64::to_u128(uq64x64::fraction(reserve_x, reserve_y)) * time_elapsed;
            lp.last_price_y_cumulative = ForbitSwapPoolV1Library::overflow_add(lp.last_price_y_cumulative, last_price_y_cumulative_delta);
        };
    ...
    fun mint_fee_interval<X, Y>(
        ...
        if (fee_on) {
            if (k_last != 0) {
                ...
                let root_k = ForbitSwapPoolV1Library::sqrt(reserve_x, reserve_y);
                let root_k_last = ForbitSwapPoolV1Library::sqrt_128(k_last);
                let total_supply = ForbitSwapPoolV1Library::get_lpcoin_total_supply<LPCoin<X, Y>>();
                if (root_k > root_k_last) {
                    let delta_k = ((root_k - root_k_last) as u128);
                    // gas saving
                    if (ForbitSwapPoolV1Library::is_overflow_mul(total_supply, delta_k)) {
    ...
    public entry fun swap_exact_coins_for_coins_entry<X, Y>(
        ...
        ForbitSwapPoolV1Library::register_coin<Y>(account);
    ...
    public entry fun swap_exact_coins_for_coins_2_pair_entry<X, Y, Z>(
        ...
        assert!(coin::value(&coins_out) >= amount_out_min, INSUFFICIENT_OUTPUT_AMOUNT);
        ForbitSwapPoolV1Library::register_coin<Z>(account);
    ...
    public entry fun swap_exact_coins_for_coins_3_pair_entry<X, Y, Z, W>(
        ...
        assert!(coin::value(&coins_out) >= amount_out_min, INSUFFICIENT_OUTPUT_AMOUNT);
        ForbitSwapPoolV1Library::register_coin<W>(account);
    ...
    public fun create_pair<X, Y>() acquires AdminData, PairInfo {
        assert!(ForbitSwapPoolV1Library::compare<X, Y>(), PAIR_ORDER_ERROR);
        ...
        // create lp coin
        let (lp_b, lp_f, lp_m) = coin::initialize<LPCoin<X, Y>>(&resource_account_signer, utf8(b"ForbitSwapLPCoin"), utf8(b"FBSLPCoin"), 8, true);
        // register coin
        ForbitSwapPoolV1Library::register_coin<LPCoin<X, Y>>(&resource_account_signer);
    ...
    public fun mint<X, Y>(
        ...
        let liquidity;
        if (total_supply == 0) {
            liquidity = ForbitSwapPoolV1Library::sqrt(amount_x, amount_y) - MINIMUM_LIQUIDITY;
            ...
            liquidity = ForbitSwapPoolV1Library::min(amount_1, amount_2);
        };
    ...
    #[test_only]
    use SwapDeployer::SpaceCoin::{Self, BTC, USDT};
    #[test_only]
    ...
    fun test_init(creator: &signer, resource_account: &signer, someone_else: &signer) acquires AdminData, PairInfo {
        ...
        // init creator
        SpaceCoin::initialize(creator);
        init_module_test(resource_account);
        // init someone_else
        SpaceCoin::register_coins_all(someone_else);
        SpaceCoin::mint_coin<BTC>(creator, signer::address_of(someone_else), INIT_FAUCET_COIN);
        SpaceCoin::mint_coin<USDT>(creator, signer::address_of(someone_else), INIT_FAUCET_COIN);
    ...
    public entry fun test_add_remove_liquidity_basic_1_1(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 10000/10000 coin and gives 9000 LPCoin (ForbitSwapPoolV1Library::sqrt(10000*10000)-1000)
    ...
    public entry fun test_add_remove_liquidity_basic_1_100(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 1000/100000 coin and gives 9000 LPCoin (ForbitSwapPoolV1Library::sqrt(1000*100000)-1000)
    ...
    public entry fun test_add_remove_liquidity_basic_1_2(creator: &signer, resource_account: &signer, someone_else: &signer) 
        ...
        // should takes 1000/2000 coin and gives 414 LPCoin (ForbitSwapPoolV1Library::sqrt(1000*2000)-1000)
    ...
    public entry fun test_flash_swap_a(creator: &signer, resource_account: &signer, someone_else: &signer, another_one : &signer)
        ...
        SpaceCoin::register_coins_all(another_one);
        SpaceCoin::mint_coin<BTC>(creator, signer::address_of(another_one), INIT_FAUCET_COIN);
        SpaceCoin::mint_coin<USDT>(creator, signer::address_of(another_one), INIT_FAUCET_COIN);
