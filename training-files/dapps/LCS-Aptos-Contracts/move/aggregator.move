module lcs_aggregator::aggregator {
    ...
    public entry fun create_aux_signer(admin: &signer){
        assert!(signer::address_of(admin) == @lcs_aggregator, E_NOT_ADMIN);
    ...
    public entry fun create_tortuga_signer(admin: &signer) {
        ...
        if (!exists<TortugaSigner>(@lcs_aggregator)){
    ...
    public entry fun init_coin_store<X>(admin: &signer) {
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == @lcs_aggregator, E_NOT_ADMIN);
        if (exists<CoinStore<X>>(@lcs_aggregator)){
            return
        };
    ...
    fun emit_swap_step_event<Input, Output>(
        ...
        let event_store = borrow_global_mut<EventStore>(@lcs_aggregator);
    ...
    fun change_coin_type<X, Y>(x_coin: coin::Coin<X>): coin::Coin<Y> acquires CoinStore {
        ...
        assert!(exists<CoinStore<X>>(@lcs_aggregator), E_COIN_STORE_NOT_EXITES);
        let amount = coin::value(&x_coin);
        let x_coin_store = borrow_global_mut<CoinStore<X>>(@lcs_aggregator);
        coin::merge(&mut x_coin_store.balance, x_coin);

        let y_coin_store = borrow_global_mut<CoinStore<Y>>(@lcs_aggregator);
        coin::extract(&mut y_coin_store.balance, amount)
    }

    #[test(admin = @lcs_aggregator)]
    ...
    public fun get_intermediate_output<X, Y, E>(
        ...
        /*
            ...
            if (is_x_to_y) {
                // sell
                market::swap_coins<X, Y>(@lcs_aggregator, market_id, false, 0, x_value, 0, HI_64, 0, &mut x_in, &mut y_out);
            }
            else {
                // buy
                market::swap_coins<Y, X>(@lcs_aggregator, market_id, true, 0, HI_64, 0, x_value, HI_64, &mut y_out, &mut x_in);
            };
        ...
        */
            ...
            if (
                ...
                let tortuga_signer = account::create_signer_with_capability(
                    &borrow_global<TortugaSigner>(@lcs_aggregator).signerCapability
                );
            ...
            let y_out = amm_router::swap<X, Y>(@lcs_aggregator, x_in);
            (option::none(),y_out)
        }
    ...
    public entry fun one_step_route<X, Y, E>(
        ...
        process_fee(&mut coin_out, fee_to, fee_bips);
    ...
    >(
        ...
        process_fee(&mut coin_z, fee_to, fee_bips);
    ...
    public fun process_fee<X>(coin: &mut Coin<X>, fee_to: address, fee_bips: u8){
        ...
        if (coin::is_account_registered<X>(fee_to)){
            if (coin::is_account_registered<X>(@lcs_aggregator)){
                coin::deposit(fee_to, coin::extract(coin, fee_value/2));
                coin::deposit(@lcs_aggregator, coin::extract(coin, fee_value/2));
            } else {
                coin::deposit(fee_to, coin::extract(coin, fee_value));
            }
        } else {
            if (coin::is_account_registered<X>(@lcs_aggregator)){
                coin::deposit(@lcs_aggregator, coin::extract(coin, fee_value));
            }
        }
    }
}
