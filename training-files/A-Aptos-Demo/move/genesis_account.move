...
module aptos_test_proj1::genesis_account {
    use aptos_framework::account;
    use aptos_framework::event;
    use aptos_test_proj1::resource_account;
    use std::error;
    use std::signer;
    ...
    public fun assert_genesis_account(account: &signer) {
        assert!(signer::address_of(account) == @aptos_test_proj1, error::invalid_argument(ENOT_GENESIS_ACCOUNT));
    }

    public(friend) fun resource_account_signer(): signer {
        resource_account::resource_account_signer(@aptos_test_proj1)
    }

    public fun resouce_account_address(): address {
        let res_account = resource_account::resource_account_signer(@aptos_test_proj1);
