...
module marketplace::marketplace_auction_example {
    ...
    // #[expected_failure]
    public fun tetest_listing_bidsst(
        lister: signer,
        bidder_a: signer,
        bidder_b: signer,
        bidder_c: signer,
        ...
        account::create_account_for_test(signer::address_of(&bidder_a));
        account::create_account_for_test(signer::address_of(&bidder_b));
        account::create_account_for_test(signer::address_of(&bidder_c));
        ...
        coin::create_fake_money(&framework, &bidder_a, 1500);
        coin::register<coin::FakeMoney>(&bidder_b);
        coin::register<coin::FakeMoney>(&bidder_c);
        coin::register<coin::FakeMoney>(&fee_account);
        coin::transfer<coin::FakeMoney>(&framework, signer::address_of(&bidder_a), 0);
        coin::transfer<coin::FakeMoney>(&framework, signer::address_of(&bidder_b), 500);
        coin::transfer<coin::FakeMoney>(&framework, signer::address_of(&bidder_c), 500);
        ...
        // highest bidder bidder B get the token
        assert!(token::balance_of(signer::address_of(&bidder_c), token_id) == 1, 1);
        assert!(token::balance_of(signer::address_of(&bidder_a), token_id) == 0, 1);
        // 3 coin is paid for market fee and remaining is 297
        assert!(coin::balance<coin::FakeMoney>(signer::address_of(&lister)) == 495, 1);
    }
}
